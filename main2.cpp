...
void status_monitoring_system(int index, item * input_array){
    if (input_array[index].amount <= 5){
        cout << input_array[index].name << "only has" << input_array[index].amount << "in stock now, it is time to re-stock!"
    }
}
